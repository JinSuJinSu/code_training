...
public class StockTestUtils {
    ...
    public static void addStock(
        ...
        if(stockType.equals("국내주식")){
            ...
            stock1 = createStock("니나브ICT", 10000L, 20L,"원",itemStandard,user);
            stock2 = createStock("샤나전자", 9000L, 15L,"원",itemStandard,user);
            stock3 = createStock("크리스틴엔지니어링", 8000L,15L,"원",itemStandard,user);
        }

        if(stockType.equals("해외주식")){
            itemStandard = domesticStockRepository.findStandard("해외주식");
            stock1 = createStock("자바컴퍼니", 1000L, 20L,"달러",itemStandard,user);
            stock2 = createStock("주식회사리액트", 900L, 15L,"달러",itemStandard,user);
            stock3 =  createStock("MSA엔지니어링", 800L,15L,"달러",itemStandard,user);
        }
        em.persist(stock1);
        em.persist(stock2);
        em.persist(stock3);
